...
class Match < ActiveRecord::Base
  def self.report
    ...
    message << "\nGoals per game: #{(matches.pluck(:my_score).inject(:+).to_f/matches.size).to_s[0..4].to_f}"
    message << "\nGoals against per game: #{(matches.pluck(:rival_score).inject(:+).to_f/matches.size).to_s[0..4].to_f}"
    message << "\nShots per game: #{(matches.pluck(:my_shots).inject(:+).to_f/matches.size).to_s[0..4].to_f}"
    message << "\nShots against per game: #{(matches.pluck(:rival_shots).inject(:+).to_f/matches.size).to_s[0..4].to_f}"
    ...
    message << "\nPass accuracy: #{(matches.pluck(:my_pass_accuracy).inject(:+).to_f/matches.pluck(:my_pass_accuracy).size).to_s[0..4].to_f}%"
    message << "\nPossession: #{(matches.pluck(:my_possession).inject(:+).to_f/matches.pluck(:my_possession).size).to_s[0..4].to_f}%"
